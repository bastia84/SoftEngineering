...
<body id="body1"
	...
	<div id="table1" style="width: 100%">
		<table id="tg-PJUO3" class="tg" style=" margin: auto; float:left;">
			<tr>
				<th class="tg-fefd">Employee</th>
				<th class="tg-fefd">Department</th>
				<th class="tg-fefd">Charge #</th>
				<th class="tg-fefd">Job Count</th>
			...
			<%
							...
							Float[] averageDept = (Float[]) request.getAttribute("averageDept");
							HashMap<String, Float> averageEmpHash = (HashMap<String, Float>) request.getAttribute("avgEmp");
							String format = "";							
							if (Worker != null) {
								System.out.println(Worker.length);
								int avg = Math.round(averageDept[0]);	
								for (z = 0; z < Worker.length; z++) {
									
									String emp = Worker[z].getName();
									int EmployeeAvg = Math.round(averageEmpHash.get(emp));
						...
						%>
			<tr>
				<td class=<%=format%>><%=emp%></td>
				<td class=<%=format%>><%=Worker[z].getDepartment()%></td>
				<td class=<%=format%>>${charge}</td>
				<td class=<%=format%>><%=Worker[z].getCount()%></td>
				<td class=<%=format%>><%=Worker[z].getHours()%></td>
				<td class=<%=format%>><%=EmployeeAvg-avg%></td>
			</tr>
			<%
							}
							}
...
<script>
	function LogOutRedirect(){
		document.location.href="RPTM.jsp";
	}
	</script>
</body>
</html>
